...
import { TemplatePath, CommandDriver } from '../config';
class ReaderDriver {
  ...
  public getFileDir(): string {
    const fileDir = workspace.getConfiguration('wulingshan').get('fileDir', store.booksPath);
  ...
  private getSearchDriver = function (onlineSite: string): string {
    return CommandDriver(onlineSite)
    switch (onlineSite) {
      case '起点':
        return CommandDriver(onlineSite);
      case '笔趣阁':
      ...
      case '追书神器':
        return './driver/zhuishushenqi';
      case '番茄':
        return './driver/fanqie';
      case '纵横':
        return './driver/zongheng';
      default:
